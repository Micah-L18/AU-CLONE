...
- The match score is a real rally score: big **+1** buttons next to each team's number (offline-queued and undoable like everything else).
- Each action carries two tunable values: **player points** go to whoever earned the action, and **team points** go to every roster teammate *except* the earner. A kill can pay the hitter 8 and each teammate 2 — all on the individual leaderboard, independent of the rally score.
